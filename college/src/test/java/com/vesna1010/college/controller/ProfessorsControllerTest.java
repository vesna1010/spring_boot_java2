...
public class ProfessorsControllerTest extends BaseControllerTest {
	...
	private void renderPageWithProfessors() throws Exception {
		...
		when(service.findAllProfessors(PAGEABLE)).thenReturn(new PageImpl<Professor>(professors));
	...
	private void renderPageWithProfessorsByStudyProgrammeId() throws Exception {
		...
		when(service.findAllProfessorsByStudyProgramId(1L, PAGEABLE)).thenReturn(new PageImpl<Professor>(professors));
	...
	private void saveProfessor() throws Exception {
		Professor professor = new Professor(1L, "Professor", "Parent", LocalDate.of(1985, Month.JANUARY, 6),
				"profesor@gmail.com", "065 123 338", Gender.MALE, "Address", getPhoto(), "Title");

		when(service.saveProfessor(professor)).thenReturn(professor);
	...
	private void rederFormWithProfessor() throws Exception {
		when(service.findProfessorById(1L)).thenReturn(new Professor(1L, "Professor"));
		...
		mockMvc.perform(
		       ...
		       .andExpect(status().isOk())
		       .andExpect(model().attribute("professor", hasProperty("name", is("Professor"))))
		       .andExpect(view().name("professors/form"));
